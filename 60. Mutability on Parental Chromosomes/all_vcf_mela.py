...
file_names = glob.glob(os.getcwd()+"/TEST/mela/*/*/*/*/*/*/*/*/*.vcf.gz", recursive=True)
print("Total number of files in MELA directory are : ", len(file_names))
...
with open("coordinates_mela.txt", "w") as f:
    ...
    for i, pair in enumerate(fnames.items()):
        ...
        x, y = proj.iloc[0,0], proj.iloc[0,1]
        ...
        G = plt.scatter(x=exon_pca.iloc[:,0], y=exon_pca.iloc[:,1], c=cn, marker='o', linewidth=0.5, s=100, alpha=0.8, zorder=1)
        f.write(f"({x}, {y}) : {ID}\n")
...
plt.savefig('images/chr_proj_all_mela_01.png', bbox_inches='tight')
plt.show()
