...
def recodonize_sequence(dna_seq):
    ...
    aa_sequence = translate(exon, table=codon_table)

    recodnized_sequence = ''
    for aa_count in range(len(aa_sequence)):
            aa = aa_sequence[aa_count]
            current_codon = exon[aa_count*3:aa_count*3+3]
            if aa == 'M' or aa == "W":
                    recodnized_sequence += current_codon
            elif len(aa_to_codon[aa]) == 2:
                    if current_codon == aa_to_codon[aa][0] :
                           recodnized_sequence += aa_to_codon[aa][1]
                    else:
                            recodnized_sequence += aa_to_codon[aa][0]

            elif len(aa_to_codon[aa]) == 3:
                    codon_index = aa_to_codon[aa].index(current_codon)
                    if codon_index == 2:
                           recodnized_sequence += aa_to_codon[aa][1]
                    else:
                           recodnized_sequence += aa_to_codon[aa][codon_index+1]
            else:
                    codon_index = aa_to_codon[aa].index(current_codon)
                    if codon_index == 3:
                           recodnized_sequence += aa_to_codon[aa][2]
                    else:
                           recodnized_sequence += aa_to_codon[aa][codon_index+1]
